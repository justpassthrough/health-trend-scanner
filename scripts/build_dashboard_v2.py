...
<div id="trendView">
  ...
  <div class="trend-container">
    <h3>📈 지속 키워드 리더보드 <span style="font-size:12px;color:#8b949e;font-weight:400;">며칠째 뜨는지 + 미니 추이</span></h3>
...
function renderTrendWindow() {{
  ...
  // ── 지속 리더보드 (등장일수순) ──
  const ranked = windowKeys.slice().sort((a, b) => (allKeyCount[b] || 0) - (allKeyCount[a] || 0));
  let lbHtml = "";
  ranked.forEach((tk, i) => {{
