...
_VERB_ENDINGS = re.compile(
    r"(하|할|한|된|된다|하는|하고|하여|했던|하며|이다|된다|인지|인가|일까|라면|라서|해야|해서|해도|되어|되는|되면|됩니다|해요|돼요|없이|있어|이어질|이어|않아|살빼|빼는|빼야|빼기|먹는|먹을|먹어|걸릴|걸리|나올|나오|줄이|늘리|줄일|늘릴|됐다|됐|했다|났다|왔다|갔다|되고|있으|하며|하면서|확대되고)$"
)
