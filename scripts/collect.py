...
_VERB_ENDINGS = re.compile(
    r"(하|할|한|된|된다|하는|하고|하여|했던|하며|이다|된다|인지|인가|일까|라면|라서|해야|해서|해도|되어|되는|되면|됩니다|해요|돼요|없이|있어|이어질|이어|않아|살빼|빼는|빼야|빼기|먹는|먹을|먹어|걸릴|걸리|나올|나오|줄이|늘리|줄일|늘릴|됐다|됐|했다|났다|왔다|갔다|되고|있으|하며|하면서|확대되고|해보세요|해보자|해봐야|알아보자|해드리겠습니다|보세요|봐야|가서|찾아|봅시다)$"
)
...
_JUNK_WORDS = {
    ...
    "수많", "많은", "많이", "다양", "실제", "적정", "맞춤", "상승",
    "풍부", "등에", "생각", "단순히", "이제", "아직", "매우",
    "중인", "소아비", "고도비", "범위", "계획", "참여",
    "있으", "되고", "하며", "하면서",
    "여기", "거기", "이해하기", "혼합색상", "상체비", "하체비",
    "톤이",
    # 부사/대명사/접속사
    "갑자기", "꾸준히", "매년", "최근", "특히", "지금",
    "제가", "저의", "자기", "우리", "그런",
    # 동사/서술형 조각
    "가서", "찾아", "해보세요", "확인해보세요", "해드리겠습니다",
    "해보자", "알아보자", "해봐야",
    # 시간/장소
    "일요일", "토요일", "월요일", "오전", "오후",
    # 비건강 명사
...
def _is_quality_keyword(kw):
    ...
    # 구두점/특수문자 포함 시 탈락 (curly quotes, hyphen 포함)
    if re.search(r"[.,;:!?…#@()[\]{}\"\u2018\u2019\u201C\u201D'`~\-]", kw):
        return False
    ...
    # 복합어: 절반 이상이 블랙리스트/불용어면 탈락
    if len(parts) >= 2:
        generic_count = sum(1 for p in parts if p in BLACKLIST or p in _STOPWORDS)
        if generic_count >= len(parts) / 2:
            return False
    ...
    if len(parts) >= 2:
        for i, p1 in enumerate(parts):
            for j, p2 in enumerate(parts):
                if i != j and len(p1) >= 2 and len(p2) >= 2 and (p1 in p2 or p2 in p1):
                    return False
